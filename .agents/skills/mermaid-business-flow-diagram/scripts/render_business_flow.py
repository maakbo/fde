#!/usr/bin/env python3
"""Validate a business flow and render self-contained SVG and PNG siblings."""
...
def main() -> int:
    ...
    linter = Path(__file__).resolve().parent / "check_business_flow.py"
    ...
    if not input_path.is_file() or not config.is_file():
        ...
        return 2
    if subprocess.run([sys.executable, str(linter), str(input_path), "--strict"], check=False).returncode != 0:
        return 1
