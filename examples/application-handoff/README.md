...
複数の開発チームが利用する共通データベースと実行環境を、アーキチームからアプリ保守チームへ引き継ぐ架空の例です。具体的な操作だけでなく、何を整える業務なのか、どの情報と判断が必要なのかを共有します。特定案件の事実や標準手順ではありません。

この仕事の目的は、開発チームが共通のデータ構造と環境を使い、アプリケーションの変更を安全に続けられる状態を保つことです。「設計」「実装」「導入」「運用」は業務を探すための工程名で、業務の親子関係や一方向の順番を表しません。

## 引き継ぐ業務

- [共有データ構造を整合させる](shared-database-context.md)
- [実行環境を利用可能に保つ](environment-management-context.md)

以下は、その中の「変更を環境へ反映する」という一つの場面を、作業レクチャーへつなぐ例です。
