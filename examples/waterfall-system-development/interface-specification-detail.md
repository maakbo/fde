...
外部IF仕様を一度で確定せず、要件・試行・証跡・外部結合結果を往復しながら、次の工程が使える契約へ成熟させるためのpilotです。

これは一つのInformationで形式を試すreader-facing viewです。他の設計Informationへ横展開するルールは、ここでは決めません。

← [外部結合テスト](external-integration-context.md) / [原因特定](external-integration-flow.md)
...
flowchart LR
  ...
  b_validate_contract@{ label: "検証", img: "https://raw.githubusercontent.com/maakbo/fde/main/assets/icons/lucide-thin/ellipse.svg", pos: "b", w: 30, h: 30, constraint: "on" }
  b_baseline_interface@{ label: "合意", img: "https://raw.githubusercontent.com/maakbo/fde/main/assets/icons/lucide-thin/ellipse.svg", pos: "b", w: 30, h: 30, constraint: "on" }
  ...
  b_rehearse_contract --- b_align_interface
  i_exchange_evidence --- b_align_interface
  b_align_interface --- b_validate_contract
  b_validate_contract --- b_baseline_interface
  b_baseline_interface --- i_external_test_specification
  b_baseline_interface --- i_external_test_result

  class b_shape_interface,b_rehearse_contract,b_align_interface,b_validate_contract,b_baseline_interface business;
...
| --- | --- | --- | --- | --- | --- |
| 項目・コード体系 | 外部IF要件、外部IF仕様 | IF設計・契約検証 | 設計中 → 整合済み | 不一致ならIF設計へ戻る | 外結仕様 |
| 順序・再送・冪等性 | 外部IF仕様、送受信証跡 | 試行・照合 | 試行で仮置き → 外結結果で確定 | 不整合なら試行・整合へ戻る | 外部結合テスト |
| 異常応答・責任境界 | 外部IF仕様、外結結果 | 検証・原因特定 | 外結結果を反映して合意 | 所在不明なら原因特定へ戻る | 外部担当・次のテスト |

この表はチェック項目の一覧ではありません。同じ`外部IF仕様`を、どの根拠から考え始め、何度見直し、どの状態で次へ渡すかを会話するための最小の読み方です。

## この図が表していること

外部IF仕様は、IF設計で書いて終わる成果物ではありません。試行と送受信証跡を通じて整合し、外部結合で得た結果を反映して、次のテストが使える合意へ成熟します。ここではその設計観点を既存Informationの詳細Viewとして表し、新しい`Design Concern` node typeは追加していません。
...
| [Information master](master-information-model.md) | `i_external_test_specification` | 外結仕様 | 合意後に外部結合で使う仕様 |
