...
次の語は、ファイル形式や保管場所ではなく、システム開発の会話で同じ意味を指すための概念です。

| ID | 用語 | 意味と境界 | 主な関係 |
| --- | --- | --- | --- |
| `i_request_background` | 依頼背景 | なぜ開発が必要か、現状の困りごとやきっかけを表す | 業務要件の前提になる |
| `i_business_requirement` | 業務要件 | 業務として必要な変化・能力を表す。システムの実現方法は含めない | 依頼背景を受け、システム要件へつながる |
| `i_system_requirement` | システム要件 | 対象システムが満たす責務・条件を表す。業務側の目的と対応づけて読む | 業務要件を受け、外部仕様・外部IF要件を導く |
| `i_external_specification` | 外部仕様 | 利用者や外部システムから見える振る舞い・境界を表す | システム要件を具体化し、テスト仕様へ渡る |
| `i_external_interface_requirement` | 外部IF要件 | 外部システムとの情報交換に必要な条件を表す | システム要件と外部IF仕様をつなぐ |
| `i_acceptance_criteria` | 受入条件 | 業務側が使えると判断するための確認条件を表す | 要件合意から受入確認へ渡る |

## 業務での扱い

| 概念 | 作成・整える場面 | 更新・確かめる場面 | 参照する場面 | 次に渡る概念 |
| --- | --- | --- | --- | --- |
| 依頼背景 | 見積・計画 | 要件定義で前提を確認する | 要件化・責務設計 | 業務要件 |
| 業務要件 | 要件定義 | 要件合意で差異を解消する | 責務設計・受入条件の整理 | システム要件 |
| システム要件 | 責務設計 | 要件合意で業務責任者と確認する | 基本設計・テスト設計 | 外部仕様、外部IF要件 |
| 外部仕様 | 基本設計 | 試行・外部結合で結果を反映する | 実装・外部結合テスト | 内部仕様、外結仕様 |
| 外部IF要件 | 責務設計 | IF設計・接続確認で条件を揃える | 外部結合テスト | 外部IF仕様 |
| 受入条件 | 要件合意 | 受入確認で実業務に照らして確かめる | 導入判断 | 受入判定、業務確認結果 |
